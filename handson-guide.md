...
- **初期Stepは全体を提示**: Step 1-2など、ファイルが新規作成される段階では全体を提示
- **後半Stepは差分形式**: Step 3以降など、既存ファイルを更新する段階では「変更点」を中心に説明
  - 冒頭に「主な変更点」を箇条書きでまとめる
  - 変更される関数・クラスのコードブロックを提示
  - `# === 追加 ===` `# 追加` などのコメントで変更箇所を明示
  - 省略部分は `# ...省略...` で示す
- **重要な変更点にはコメント**: `# ← 追加` など
